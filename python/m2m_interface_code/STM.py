# CONTROL logic for operating the stepper motor
# PROTOCOL: Abstracted using commands of RPI_I2C_ARDUINO - interface in terms of command strings
...
def Stepper(position, speed, sample_length, position_num):
	...
	# DISTANCE Calculations + format distance for instruction
	# 110.3 is the diameter of the motor wheel
	distance_ = int(float(x - position - sample_length)*(3200/(math.pi*110.4)))
	distance_ = str(distance_)
	Distance = distance_ + 'Dis'

	# SPEED Calculations + format speed for instruction
	# convert speed unit rpm to stepper motor's microsteps/sec
	speed_ = int(float(speed)*(3200/60))
	speed_ = str(speed_)
	Speed = speed_ + 'Spd'

	# POSITION stage number + format for instruction
	position = str(position_num) + 'P'

	# DIRECTION setting + format direction for instruction
	direction = '1' + 'E'# 'E' stands for end.

	cmd = Distance + Speed + position + direction
	communication_send(addr, "stm", cmd)

# STM_T is a frame for stepper motor testing
class STM_T(wx.Frame):
