...
from python.m2m_interface_code.solenoid_thread_test import *

from python.ui_components import mygrid, panelFive, panelThree, tempSettingTwo, panelOne, panelTwo, tempSettingOne, \
	panelFour, GasQuenching
from python.m2m_interface_code.STM import *
from python.m2m_interface_code.RPI_I2C_ARDUINO import *
from python.m2m_interface_code.FastRead import *
...
#CONVERT String Representation To An Integer
def converttoint(s):
...
# CONVERT Time interval list into a list of time coordinates for drawing the presets
def newlist(l):
...
#CONVERT A number to a n-digit data value
...
#---------------------------------------------------------------------------------------------
	...
	def onShowPlot(self,event):
		filename1 = "results/" + self.TestInfo.GetValue()+ ' Heating' + '.csv'
		filename2 = "results/" + self.TestInfo.GetValue()+ ' Cooling' + '.csv'
		if self.Furnace2Lock == 1:
			filename3 = "results/" + self.TestInfo.GetValue()+ ' Reheating' + '.csv'
			filename4 = "results/" + self.TestInfo.GetValue()+ 'Furnace Cool' + '.csv'
		...
		OutputFileName = "results/" + self.TestInfo.GetValue() + ' total.csv'
