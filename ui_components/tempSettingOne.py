...
class TabPanel(wx.Panel):
	...
	def UpdateText(self, event):
		...
		if (event.GetId()<=self.number_of_buttons + 500 and event.GetId()>=501):
			self.object2 = event.GetEventObject()

			self.setTime[event.GetId()-501] = getSec(self.object2.GetValue())
	...
	def onAddWidget(self, event):
		...
		self.new_Temp = wx.lib.intctrl.IntCtrl(self, ID, size=(40, -1),style=wx.TE_RIGHT, name=name)
		self.new_Temp_Unit = wx.StaticText(self,ID , label = "C", name=name)
		self.new_Interval = masked.TimeCtrl(self, ID + 500 , -1 , fmt24hr=True, name=name)
	...
	def onRemoveWidget(self, event):
		if self.widgetSizer.GetChildren():
			...
			self.setTime[self.nubmer_of_buttons] = None
			self.frame.Fit()
